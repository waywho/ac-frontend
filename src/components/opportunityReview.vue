<template>
  <div class="review-panel">
  	<div class="post-review">
  		<div class='post-header large is-golden'><b>{{opp.company}}</b></div>
		  <p class="big is-golden"><b>{{opp.title}}</b></p>

		  <span class="smaller">Posted: {{today | moment("MMMM DD, YYYY")}}</span>
  		<div class="row is-golden">
  		...
  		<p v-if="opp.min_age !== null || opp.max_age !== null" class="is-golden">{{opp.min_age}}-{{opp.max_age}}</p>
  		<h5>Application Deadline</h5>
  		<p v-if="opp.deadline !== null" class="is-golden">{{opp.deadline | moment("MMMM DD, YYYY")}}</p>
  		...
  		<ul v-if="opp.audition_dates.length > 1" v-for="audition in opp.audition_dates">
  			<li class="is-golden">{{audition.location}} - {{audition.from | moment("MMMM DD, YYYY")}}</li>
  		</ul>
  		<h5 v-if="opp.materials_required !== null">Materials Required</h5>
  		<p v-if="opp.materials_required !== null" class="post-text is-golden">{{opp.materials_required}}</p>
  		<h5>Opportunity Description</h5>
  		<p class="post-text is-golden">{{opp.description}}</p>
  		<h5>Payment</h5>
  		<p class="post-text is-golden">{{opp.payment_details}}</p>
  		<h5>Contact Information</h5>
  		<p class="post-text is-golden">{{opp.contact_information}}</p>
  	</div>
